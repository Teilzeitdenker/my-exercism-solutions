...
let winner (board: Board) : Player option =
    ...
    let rec isConnected r c stone visited toPlayerGoal =
        if visited |> List.contains (r, c) then false
            ...
            if not (onBoard r c) || board[r][c] <> stone then false
                ...
                isConnected (r + dr) (c + dc) stone ((r, c)::visited) toPlayerGoal)
    let toBlackGoal (r, c) = c = r + 2 * (width - 1)
    let toWhiteGoal (r, _) = r = height - 1 
    if  [0..1..(height - 1)] |> Seq.exists (fun r -> isConnected r r 'X' [] toBlackGoal) then
        Some Black
    elif [0..2..2*(width-1)] |> Seq.exists (fun c -> isConnected 0 c 'O' [] toWhiteGoal) then 
        Some White 
    else None