...
// collects for every given character in the grid (the key of the map) a list of its index positions (the value of the map) 
let gridToMap (grid : string list) : Map<char,(int * int) list> =
    grid 
    |> Seq.indexed 
    |> Seq.map (fun (i, s) -> 
        s 
        |> Seq.indexed 
        |> Seq.map (fun (j, c) -> 
            seq { (c, [(j+1, i+1)]) } |> Map.ofSeq 
        )
        |> Seq.fold (fun acc m -> merge acc m (fun _ v1 v2 -> v1 @ v2)) Map.empty
     )
     |> Seq.fold (fun acc m -> merge acc m (fun _ v1 v2 -> v1 @ v2)) Map.empty

// fetches all possible starting positions from the gridMap by looking at the first character of the word
// then enriches every starting position with all possible 8 directions and then uses checkDirection from index 1 
...
let checkWord (gridMap : Map<char,(int * int) list>) (word : string) : string * Option<((int * int) * (int * int))> = 
    let rec checkDirection start direction index = 
        if index = String.length word then true 
        else 
            let position = addTuples start (scalarMult index direction) 
            if gridMap[word[index]] |> Seq.contains position then checkDirection start direction (index + 1)
            else false 
    ...
    if word |> Seq.exists(fun ch -> Map.containsKey ch gridMap |> not) then (word, None)
        ...
        let results = 
            ...
            |> Seq.filter (fun (start, dir) -> checkDirection start dir 1)
        match results |> Seq.tryHead with 
            | None -> (word, None)
            | Some (start, dir) -> 
                let endpos = addTuples start (scalarMult (String.length word - 1) dir) 
