defmodule WordSearch do
  ...
  defp grid_to_map(grid) do
    grid |> String.split("\n", trim: true) |> Enum.map(&String.trim/1) |> Enum.with_index(1)
    |> Enum.map(fn {s, row} ->
      s |> String.codepoints |> Enum.with_index(1)
  ...
  defp check_word(grid_map, word) do
    if word |> String.codepoints() |> Enum.any?(fn ch -> !Map.has_key?(grid_map, ch) end) do
      ...
      results =
        ...
        |> Enum.filter(fn {start, direction} -> check_direction(start, direction, grid_map, word, 1) end)
  ...
  # @spec check_direction(start :: {integer, integer}, direction :: {integer, integer}, grid_map :: %{String.t() => [{integer, integer}]}, word :: String.t(), index :: integer) :: bool
  defp check_direction(start, direction, grid_map, word, index) do
    if index == String.length(word) do
      true
    else
      character = word |> String.at(index)
      position = add_tuples(start, scalar_mult(index, direction))
      if Map.fetch!(grid_map, character) |> Enum.member?(position), do: check_direction(start, direction, grid_map, word, index + 1), else: false
    end
  end

  # Small helper functions for addition and scalar multiplication of index tuples
  defp add_tuples({a1, b1}, {a2, b2}), do: {a1 + a2, b1 + b2}
  defp scalar_mult(k, {a, b}), do: {k*a, k*b}
end
